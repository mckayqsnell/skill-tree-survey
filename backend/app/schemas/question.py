...
class QuestionBase(BaseModel):
    ...
    parent_id: Optional[int] = Field(None, description="Parent question ID")
    is_base: bool = Field(False, description="Whether this is a base question")
    category: Optional[str] = Field(None, max_length=50, description="Question category")
...
class QuestionReorder(BaseModel):
    ...
    parent_id: Optional[int] = Field(None, description="Parent ID (None for base questions)")
    question_ids: List[int] = Field(..., min_length=1, description="Ordered list of question IDs")


class QuestionMove(BaseModel):
    """Schema for moving a question."""
    question_id: int = Field(..., description="Question to move")
    new_parent_id: Optional[int] = Field(None, description="New parent (None for base level)")
