...
class SessionDAO(BaseDAO[SurveySession]):
    ...
    def get_with_responses(self, session_id: int) -> Optional[SurveySession]:
        """
    ...
    def get_incomplete_sessions(self, older_than_hours: int = 24) -> List[SurveySession]:
        """
    ...
    def mark_completed(self, session_id: int) -> Optional[SurveySession]:
        """
    ...
    def get_analytics(self, company: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        completed_sessions = query.filter(SurveySession.completed_at.isnot(None)).count()
    ...
    def get_user_skills_summary(self, email: str) -> Dict[str, Any]:
        ...
        latest_completed = next(
            (s for s in sessions if s.is_completed()),
            None
        )
        ...
        if latest_completed:
            ...
            yes_responses = [
                r for r in latest_completed.responses if r.answer
            ]
