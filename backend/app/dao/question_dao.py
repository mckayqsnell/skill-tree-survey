...
class QuestionDAO(BaseDAO[Question]):
    ...
    def get_base_questions(self, category: Optional[str] = None) -> List[Question]:
        """
        ...
        """
        query = self.db.query(Question).filter(Question.is_base == True)
    ...
    def get_with_children(self, id: int) -> Optional[Question]:
        """
    ...
    def reorder_questions(self, parent_id: Optional[int], new_order: List[int]) -> bool:
        """
    ...
    def move_question(self, question_id: int, new_parent_id: Optional[int]) -> Optional[Question]:
        """
