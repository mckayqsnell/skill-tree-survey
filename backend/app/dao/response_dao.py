...
class ResponseDAO(BaseDAO[Response]):
    ...
    def get_by_question(self, question_id: int) -> list[Response]:
        ...
        """
        return self.db.query(Response).filter(Response.question_id == question_id).all()

    def get_response(self, session_id: int, question_id: int) -> Response | None:
        """
    ...
    def create_response(
        self, session_id: int, question_id: int, answer: bool
    ) -> Response:
        """
        ...
        for response in responses:
            if response.answer:  # Only count "yes" responses as skills
                ...
                if category not in max_depths_by_category:
                    ...
                    max_depths_by_category[category] = max(
                        max_depths_by_category[category], depth
                    )
